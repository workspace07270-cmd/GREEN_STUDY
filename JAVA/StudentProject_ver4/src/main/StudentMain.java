...
public class StudentMain {
	...
	public static void main(String[] args) {
		...
		Scanner sc = new Scanner(System.in);
						// 데이터 파일 경로 (프로젝트 루트의 student.csv, 필요시 절대경로로 변경)
						String dataFile = "student.csv";

						// 프로그램 시작 시 CSV에서 데이터 로드
						StudentService.getInstance().loadFromCSV(dataFile);
						System.out.println("데이터를 '" + dataFile + "'에서 로드했습니다. 총 " + StudentService.getInstance().getList().size() + "건");
		...
		while(true) { 
			...
			if(no == 0) {
				...
				// 프로그램 종료 직전에 현재 리스트를 CSV로 저장
				StudentService.getInstance().saveToCSV(dataFile);
				System.out.println("데이터를 '" + dataFile + "'에 저장했습니다.");
				break;
			}
