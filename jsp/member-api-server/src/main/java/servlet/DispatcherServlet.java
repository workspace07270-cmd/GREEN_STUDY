...
/**
 * 모든 사용자 요청(*.do)을 중앙에서 관리하는 입구(프론트 컨트롤러)입니다.
 * 이 서블릿이 먼저 모든 요청을 받은 뒤, 적절한 컨트롤러로 일감을 배분합니다.
 */
@WebServlet("*.do")
public class DispatcherServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(controller != null)
			resultData = (Map<String, Object>)controller.execute(request, response);
		System.out.println(resultData);
		System.out.println(new JSONObject(resultData).toString());
		response.getWriter().println(new JSONObject(resultData).toString());
	}
